...
_VERBOSE = flags.DEFINE_bool('verbose', False, 'Include extra information during execution.')
...
def main(argv):
  ...
  # Part 1.
  yline = 2000000
  intervals = [nobeacon_interval(sensor=s, beacon=b, yline=yline)
  ...
  for y in range(n+1):
    ...
    intervals = [nobeacon_interval(sensor=s, beacon=b, yline=y)
                 for (s, b) in zip(sensors, beacons)]
